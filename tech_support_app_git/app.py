...
def book():
    ...
    if file:
        filename = f"{whatsapp}_{file.filename}"
    ...
    new_booking = Booking(
        user_id=current_user.id, name=name, whatsapp=whatsapp, email=email, 
        license_no=license_no, description=description, scheduled_time=scheduled_time,
        screenshot_path=screenshot_path, razorpay_order_id=order['id']
    )
